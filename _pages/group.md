---
layout: archive
title: "Group"
permalink: /group/
author_profile: true
---

I feel very lucky to work with many smart, productive and cool people in different institutes in my career. At SJTU, I have built my own research group made of excellent students. We welcome new and young people interested in astronomy and astrophysics to join us.

**Graduates**

<div style="display: flex; gap: 20px; flex-wrap: wrap;">

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_XuejieLi.jpg" alt="Xuejie Li" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Xuejie Li (2022-, co-supervised with Prof. Xiaohu Yang) </strong>
    <br>
    PhD Candidate
    <br>
    Research: 
    <a href="https://ui.adsabs.harvard.edu/search/fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq_database=(database%3Aastronomy%20OR%20database%3Aphysics)&p_=0&q=%20author%3A%22Zhongxu%20Zhai%22%20%20author%3A%22Xuejie%20Li%22%20%20year%3A(2022-)&sort=date%20desc%2C%20bibcode%20desc" target="_blank">Our Papers</a>
  </div>
  <br><br> 

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_WenhaoGao.jpg" alt="Wenhao Gao" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Wenhao Gao (2023-)</strong>
    <br>
    PhD Candidate
    <br>
    Research: 
    <a href="https://ui.adsabs.harvard.edu/search/fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq_database=(database%3Aastronomy%20OR%20database%3Aphysics)&p_=0&q=%20author%3A%22Zhongxu%20Zhai%22%20%20author%3A%22Wenhao%20Gao%22%20%20year%3A(2022-)&sort=date%20desc%2C%20bibcode%20desc" target="_blank">Our Papers</a>
  </div>
  <br><br> 
  
  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_ZilanXiao.jpg" alt="Zilan Xiao" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Zilan Xiao (2023-)</strong>
    <br>
    PhD Candidate
    <br>
    Research:
    <a href="https://ui.adsabs.harvard.edu/search/fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq_database=(database%3Aastronomy%20OR%20database%3Aphysics)&q=%20author%3A%22Zhongxu%20Zhai%22%20%20author%3A%22Zilan%20Xiao%22%20%20year%3A(2022-)&sort=date%20desc%2C%20bibcode%20desc&p_=0" target="_blank">Our Papers</a>
  </div>
  <br><br> 

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_JunyuHua.jpg" alt="Junyu Hua" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Junyu Hua (2024-, co-supervised with Prof. Ying Zu)</strong>
    <br>
    PhD Candidate
    <br>
    Research:
  </div>
  <br><br> 

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_HaicongXu.jpg" alt="Hailing Xu" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Haicong Xu (2024-)</strong>
    <br>
    PhD Candidate
    <br>
    Research:
  </div>
  <br><br> 

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_ZehongQiu.jpg" alt="Zehong Qin" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Zehong Qiu (2024-)</strong>
    <br>
    PhD Candidate
    <br>
    Research:
  </div>
  <br><br> 

  <div style="text-align: center; margin-bottom: 40px; width: 150px;">
    <img src="/images/student_YuyangLi.jpg" alt="Yuyang Li" style="width: 120px; height: 150px; border-radius: 50%; object-fit: cover;">
    <br>
    <strong>Yuyang Li (2025-, co-supervised with Pro. Zheng Zheng at Utah)</strong>
    <br>
    PhD student
    <br>
    Research:
  </div>
  <br><br> 
  
</div>



**Current and former undergraduates**

Yechun Yong (2023-)

Junxi Zhong (2024-)
